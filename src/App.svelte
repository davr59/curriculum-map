<script>
  ...
  let resultNode;
	...
	function buildResultNodeSvg(container, width, height) {
		return container.append('svg').attr('width', width).attr('height', height).call(responsivefy);
	}

	function buildCourses(group, data, width, height, blockIndex) {
		group.selectAll().data(data).enter().append('text').transition().duration(500).text(c => c.code).attr('x', 50).attr('y', (d, i) => 25 + (i * 50));
		group.selectAll().data(data).enter().append('text').transition().duration(500).text(c => c.name).attr('x', 50).attr('y', (d, i) => 50 + (i * 50));
	}
	...
	onMount(async () => {
		...
		for (let i = 0; i < data.length; i++) {
			...
			buildCourses(group, data[i], width, height, i);
